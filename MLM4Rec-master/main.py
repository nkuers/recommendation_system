...
def main():
    ...
    parser.add_argument("--eval_mode", default="uni100", type=str, choices=["uni100", "full"])
    parser.add_argument("--num_neg", default=100, type=int)
    ...
    if args.do_eval:
        if args.load_model is None:
            ...
            scores, result_info = trainer.test(0, full_sort=(args.eval_mode == "full"))

    else:
        early_stopping = EarlyStopping(args.checkpoint_path, patience=args.patience, verbose=True)
        for epoch in range(args.epochs):
            trainer.train(epoch)
            scores, _ = trainer.valid(epoch, full_sort=(args.eval_mode == "full"))
            # evaluate on MRR
