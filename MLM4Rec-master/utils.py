...
sequential_data_list = [
    'Beauty', 'Sports_and_Outdoors', 'Toys_and_Games', 'Yelp', 'ml-1m',
    'Beauty_ours', 'BeautyLong_ours', 'Electronics_ours', 'ML100K_ours', 'Sports_ours', 'SportsLong_ours', 'Toys_ours', 'ToysLong_ours', 'Toys50K_ours'
]
...
def mrr_k(actual, predicted, topk):
    res = 0.0
    for user_id in range(len(actual)):
        target = actual[user_id]
        rank = None
        for j, item in enumerate(predicted[user_id][:topk]):
            if item == target:
                rank = j + 1
                break
        if rank is not None:
...
def get_seq_dic(args):
    if args.data_name in sequential_data_list:
        ...
        raise ValueError(
            f"Unknown data_name={args.data_name}. "
            f"Supported sequential={sequential_data_list}, session={session_based_data_list}"
        )
...
def get_dataloader(args, seq_dic):
    if args.data_name in sequential_data_list:
        ...
        eval_samples, test_samples = None, None
        if getattr(args, 'eval_mode', 'uni100') == 'uni100':
            rng = np.random.RandomState(getattr(args, 'eval_seed', 42))
            num_neg = int(getattr(args, 'num_neg', 100))
            max_item = int(args.item_size - 1)
            eval_samples = []
            test_samples = []
            for seq in seq_dic['user_seq']:
                if len(seq) < 3:
                    eval_samples.append([1] * num_neg)
                    test_samples.append([1] * num_neg)
                    continue

                eval_forbid = set(seq[:-1]) | {0}
                test_forbid = set(seq) | {0}

                eval_negs = []
                while len(eval_negs) < num_neg:
                    item = int(rng.randint(1, max_item + 1))
                    if item in eval_forbid:
                        continue
                    eval_forbid.add(item)
                    eval_negs.append(item)
                eval_samples.append(eval_negs)

                test_negs = []
                while len(test_negs) < num_neg:
                    item = int(rng.randint(1, max_item + 1))
                    if item in test_forbid:
                        continue
                    test_forbid.add(item)
                    test_negs.append(item)
                test_samples.append(test_negs)

        eval_dataset = FMLPRecDataset(args, seq_dic['user_seq'], test_neg_items=eval_samples, data_type='valid')
        eval_sampler = SequentialSampler(eval_dataset)
        eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size)

        test_dataset = FMLPRecDataset(args, seq_dic['user_seq'], test_neg_items=test_samples, data_type='test')
