...
def convert(src: Path, dst: Path):
    ...
    # Stable user mapping by numeric order when possible, otherwise lexicographic.
    def _user_key(u):
        try:
            return (0, int(u))
        except Exception:
    ...
    total_inters = sum(len(s) for _, s in kept)
    print(f'[DONE] {dst}')
    print(f'  users={len(kept)} items={len(item_map)} inters={total_inters}')
