...
class SASRecModel(nn.Module):
    ...
    def add_position_embedding(self, sequence, augment_sequence=None, alpha=None):
        ...
        if augment_sequence is not None:
            item_embeddings_org = self.item_embeddings(sequence)
            item_embeddings_aug = self.item_embeddings(augment_sequence)
            item_embeddings = alpha * item_embeddings_org + (1 - alpha) * item_embeddings_aug
    ...
    def transformer_encoder(self, input_ids, augment=None, alpha=None, perturbed=False):
        ...
        sequence_emb = self.add_position_embedding(input_ids, augment, alpha)
