...
def main():
    ...
    if args.do_eval:
        ...
        scores, result_info = trainer.test(0, full_sort=(args.eval_mode in {"full", "uni100"}))

    else:
        if args.load_pretrain:
            print('Performance of pre-trained model on the validation set:')
            trainer.valid(0, full_sort=(args.eval_mode in {"full", "uni100"}))
        early_stopping = EarlyStopping(args.checkpoint_path, patience=args.patience, verbose=True)
        for epoch in range(args.epochs):
            trainer.train(epoch)
            if epoch > args.start_valid:
                scores, _ = trainer.valid(epoch, full_sort=(args.eval_mode in {"full", "uni100"}))
