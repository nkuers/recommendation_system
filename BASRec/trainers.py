...
class Trainer:
    ...
    def get_full_sort_score(self, epoch, answers, pred_list):
        ...
        mrr10 = self._mrr_at_k(answers, pred_list, 10)
        post_fix = {
            "Epoch": epoch,
    ...
    def _mrr_at_k(self, answers, pred_list, topk):
        total = 0.0
        n = len(pred_list)
        for i in range(n):
            gt = set(np.asarray(answers[i]).reshape(-1).tolist())
            rr = 0.0
            for rank, item in enumerate(pred_list[i][:topk], start=1):
                if int(item) in gt:
                    rr = 1.0 / rank
                    break
            total += rr
    ...
    def sample_uni100_ranks(self, rating_pred, batch_user_index, answers, rng):
        ranks = []
        item_hi = self.model.args.item_size
        num_neg = int(self.args.num_neg)
        train_matrix = self.args.train_matrix
        for i, uid in enumerate(batch_user_index):
            pos = int(np.asarray(answers[i]).reshape(-1)[0])
            if pos <= 0:
                continue
