...
def main():
    ...
    parser.add_argument("--eval_mode", type=str, default="full", choices=["full", "uni100"])
    parser.add_argument("--num_neg", type=int, default=100, help="number of negatives in uni100 mode")
    parser.add_argument("--eval_seed", type=int, default=None, help="sampling seed for uni100 evaluation")
    ...
    parser.add_argument("--pretrained_path", type=str, default="", help="optional checkpoint path for TTA")

    args = parser.parse_args()
    assert args.TTA_type in ['TMask-R', 'TMask-B', 'TNoise']
    ...
    if args.TTA:
        ...
        if args.pretrained_path:
            ...
            if not os.path.exists(pretrained) and os.path.exists(args.checkpoint_path):
        ...
        if args.TTA_type == 'TMask-R' or args.TTA_type == 'TMask-B':
            trainer.test_aug(test_dataloader_tta)
        else:
            trainer.test_aug(test_dataloader)
        exit()
