...
class LRUTrainer(BaseTrainer):
    ...
    def calculate_metrics(self, batch):
        if self.args.dataset_code != 'xlong' and self.args.eval_mode == 'uni100':
            seqs, candidates, labels = batch
            scores = self.model(seqs)[0][:, -1, :]
            cand_scores = scores.gather(1, candidates)
            metrics = sampled_recall_mrr_ndcg_for_ks(cand_scores, labels, self.metric_ks)
            return metrics
        else:
            seqs, labels = batch
