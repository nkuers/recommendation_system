...
def sampled_recall_mrr_ndcg_for_ks(scores, labels, ks):
    metrics = {}
    labels_float = labels.float()
    answer_count = labels_float.sum(1)
    rank = (-scores).argsort(dim=1)
    ...
    for k in sorted(ks, reverse=True):
        ...
        hits = labels_float.gather(1, cut)
        metrics['Recall@%d' % k] = (
            hits.sum(1) / torch.clamp(answer_count, min=1.0)
        ).mean().cpu().item()
        ...
        idcg = torch.Tensor([weights[:min(int(n), k)].sum()
                             for n in answer_count]).to(dcg.device)
        idcg = torch.clamp(idcg, min=1e-12)
        ndcg = (dcg / idcg).mean()
        metrics['NDCG@%d' % k] = ndcg.cpu().item()
