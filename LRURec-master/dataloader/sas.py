...
class SASDataloader():
    def __init__(self, args, dataset):
        self.args = args
        self.rng = np.random.RandomState(args.eval_seed)
...
class SASValidDataset(data_utils.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.args.eval_mode == 'uni100':
            negs = self._sample_negs(seq, answer)
    ...
    def _sample_negs(self, seq, answer):
        seen = set(seq) | set(answer) | {0}
        negs = []
        while len(negs) < self.args.num_neg:
            item = int(self.rng.randint(1, self.args.num_items + 1))
            if item in seen:
                continue
            seen.add(item)
            negs.append(item)
